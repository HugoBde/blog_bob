...
use pulldown_cmark::{Options as MarkdownOptions, Parser};
...
impl Bob {
    pub fn new(config_path: &str) -> Result<Bob, Box<dyn Error>> {
        ...
        inotify.watches().add(&monitor_dir, WatchMask::ALL_EVENTS | WatchMask::ONLYDIR)?;

        let mut db_config = Config::new();

        db_config
            .host(config_parser.get("database", "host").unwrap_or(String::from("localhost")).as_str())
            .port(config_parser.getuint("database", "port")?.unwrap_or(5432) as u16)
            .user(config_parser.get("database", "user").ok_or("Missing \"user\" parameter in \"database\" section")?.as_str())
            .password(
                config_parser
    ...
    pub fn run(&mut self) {
        ...
        println!("Bob monitoring {} and writing to {}...", self.monitor_dir.display(), self.output_dir.display());
    ...
    fn process_events(&mut self, events: Events<'_>) -> Result<(), Box<dyn Error>> {
        ...
        for event in events {
            ...
            println!("event {:?} in {}", event.mask, elapsed.as_micros());
        }
    ...
    fn update_article(&mut self, event: &Event<&OsStr>) -> Result<(), Box<dyn Error>> {
        ...
        let frontmatter_content = parts.nth(1).ok_or(format!("Missing frontmatter in {}", input_file_name.display()))?;
        ...
        self.db_client.execute(
            ...
            &[&article_metadata.title, &output_file_name.to_str(), &article_metadata.pub_date, &article_metadata.tags],
        )?;
    ...
    fn delete_article(&mut self, event: &Event<&OsStr>) -> Result<(), Box<dyn Error>> {
        ...
        self.db_client.execute("DELETE FROM blog_articles WHERE path ~ 'shit';", &[/* &file_name.to_str() */])?;

        println!("Deleted article \"{}\"", event.name.unwrap().to_string_lossy());
